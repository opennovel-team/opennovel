...
|type: gui;                      |押されると GUI にジャンプするボタンです。                                                                                                   |
...
この場合、ジャンプ先から元のメッセージに戻るには、<code>@return</code> を使います。

`gosub: yes;` の代わりに `gosub-back: yes;` を使えば、複数のメニュー画面やシナリオファイルを経由した場合でも、`@return` で元のメッセージに戻ることができます。

`gosub: yes;` の代わりに `gosub-gui: yes;` を使えば、`@return` で元のGUIに戻ることができます。
