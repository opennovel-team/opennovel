...
/*
 * The Graphics HAL for Direct3D 11 and 10 Family
 */
...
BOOL D3D11ResizeWindow(int nOffsetX, int nOffsetY, float scale)
{
    ...
    g_fDisplayWidth = (float)conf_game_width * scale;
    g_fDisplayHeight = (float)conf_game_height * scale;

    float ratioX = (float)conf_game_width / (g_fDisplayWidth + (float)nOffsetX * 2.0f);
    float ratioY = (float)conf_game_height / (g_fDisplayHeight + (float)nOffsetY * 2.0f);

    D3D11_VIEWPORT vp;
    vp.TopLeftX = (float)nOffsetX * ratioX;
    vp.TopLeftY = (float)nOffsetY * ratioY;
    vp.Width = (float)conf_game_width - (float)nOffsetX * ratioX * 2.0f;
    vp.Height = (float)conf_game_height - (float)nOffsetY * ratioY * 2.0f;
    vp.MinDepth = 0.0f;
    vp.MaxDepth = 1.0f;
    ...
    sci.left = 0;
    sci.top = 0;
    sci.right = conf_game_width - (int)((float)nOffsetX / ratioX);
    sci.bottom = conf_game_height - (int)((float)nOffsetY / ratioY);
...
{
    DrawPrimitive2D(0, 0, conf_game_width, conf_game_height, src_image, rule_image, 0, 0, conf_game_width, conf_game_height, threshold, PIPELINE_RULE);
}

VOID D3D11RenderImageMelt(struct image *src_image, struct image *rule_image, int progress)
{
    DrawPrimitive2D(0, 0, conf_game_width, conf_game_height, src_image, rule_image, 0, 0, conf_game_width, conf_game_height, progress, PIPELINE_MELT);
}
...
static VOID DrawPrimitive2D(int dst_left, int dst_top, int dst_width, int dst_height, struct image* src_image, struct image* rule_image, int src_left, int src_top, int src_width, int src_height, int alpha, int pipeline)
{
    ...
    float x1 = (float)dst_left / ((float)conf_game_width / 2.0f) - 1.0f;
    float y1 = 1.0f - (float)dst_top / ((float)conf_game_height / 2.0f);
    float x2 = (float)(dst_left + dst_width) / ((float)conf_game_width / 2.0f) - 1.0f;
    float y2 = 1.0f - (float)dst_top / ((float)conf_game_height / 2.0f);
    float x3 = (float)dst_left / ((float)conf_game_width / 2.0f) - 1.0f;
    float y3 = 1.0f - (float)(dst_top + dst_height) / ((float)conf_game_height / 2.0f);
    float x4 = (float)(dst_left + dst_width) / ((float)conf_game_width / 2.0f) - 1.0f;
    float y4 = 1.0f - (float)(dst_top + dst_height) / ((float)conf_game_height / 2.0f);
...
static VOID DrawPrimitive3D(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, struct image *src_image, int src_left, int src_top, int src_width, int src_height, int alpha, int pipeline)
{
    ...
    float x1_ = x1 / ((float)conf_game_width / 2.0f) - 1.0f;
    float y1_ = 1.0f - y1 / ((float)conf_game_height / 2.0f);
    float x2_ = x2 / ((float)conf_game_width / 2.0f) - 1.0f;
    float y2_ = 1.0f - y2 / ((float)conf_game_height / 2.0f);
    float x3_ = x3 / ((float)conf_game_width / 2.0f) - 1.0f;
    float y3_ = 1.0f - y3 / ((float)conf_game_height / 2.0f);
    float x4_ = x4 / ((float)conf_game_width / 2.0f) - 1.0f;
    float y4_ = 1.0f - y4 / ((float)conf_game_height / 2.0f);
