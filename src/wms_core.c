...
static void free_arg_list(struct wms_arg_list *arg_list);
...
static void
free_arg_list(struct wms_arg_list *arg_list)
{
	assert(arg_list != NULL);

	free_expr(arg_list->list);
}
	...
	const char *argv[])
{
	...
	for (i = 0; i <argc; i++) {
		...
		memset(term, 0, sizeof(struct wms_term));
		term->type.is_str = 1;
		term->val.s = strdup(argv[i]);
		RT_MEM_CHECK(term->val.s);
	...
	ret = eval_func(rt, f, arg_list, &val);
