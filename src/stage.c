...
/* メッセージボックスのhoverイメージ */
static struct image *msgbox_hover_image;
...
static bool setup_msgbox(void)
{
	...
	if (msgbox_hover_image != NULL) {
		destroy_image(msgbox_hover_image);
	...
	/* メッセージボックスの画像を読み込む */
	if (conf_msgbox_hover_image != NULL) {
		msgbox_hover_image = create_image_from_file(CG_DIR, conf_msgbox_hover_image);
		if (msgbox_hover_image == NULL)
			return false;
	}
