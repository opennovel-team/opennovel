...
- (BOOL)copyResourceTemplate:(NSString *)from {
    NSArray *subfolderArray = @[@"/anime", @"/bg", @"/bgm", @"/cg", @"/ch", @"/cv", @"/font", @"/gui", @"/rule", @"/se", @"/txt", @"/wms", @"/project.txt"];
...
- (IBAction)onMenuExportForWeb:(id)sender {
    ...
    if (![fileManager createDirectoryAtURL:[NSURL fileURLWithPath:[exportPath stringByAppendingString:@"/mov"]] withIntermediateDirectories:YES attributes:nil error:nil]) {
    ...
    for (NSString *item in contents) {
        if ([[item pathExtension] isEqualToString:@"mp4"]) {
            if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/mov/%@", [fileManager currentDirectoryPath], item]
                                      toPath:[NSString stringWithFormat:@"%@/mov/%@", exportPath, item]
                                       error:nil]) {
...
- (IBAction)onMenuExportForAndroid:(id)sender {
    ...
    if (![fileManager createDirectoryAtURL:[NSURL fileURLWithPath:[exportPath stringByAppendingString:@"/app/src/main/assets/mov"]] withIntermediateDirectories:YES attributes:nil error:nil]) {
        log_warn("mkdir error (2).");
        return;
    }
    ...
    for (NSString *item in contents) {
        if ([[item pathExtension] isEqualToString:@"mp4"]) {
            if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/mov/%@", [fileManager currentDirectoryPath], item]
                                      toPath:[NSString stringWithFormat:@"%@/app/src/main/assets/mov/%@", exportPath, item]
                                       error:nil]) {
                log_warn("Copy error (3).");
                return;
            }
