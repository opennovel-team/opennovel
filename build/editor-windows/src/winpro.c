...
/* HAL Implementaions */
#include "microsoft/d3drender.h"	/* Graphics HAL */
...
static VOID OnExportLinux(void);
...
static BOOL InitApp(HINSTANCE hInstance, int nCmdShow)
{
	...
	if (!D3DInitialize(hWndRender, TRUE))
	{
	...
	if (__argc >= 2)
	{
		/* Set a startup file and line if specified in argv[2] and argv[3]. */
		if (__argc >= 4)
		{
		...
		/* Set a project file to argv[1] */
		if (OpenProjectAtPath(__wargv[1]))
		{
			StartGame();
		}
	}
	else if (FILE_EXISTS("project.txt"))
	{
		GetCurrentDirectory(sizeof(wszProjectDir) / sizeof(wchar_t), wszProjectDir);

		/* Open a game automatically if a current directory contains the project.txt file. */
		StartGame();
	}
...
static VOID InitMenu(HWND hWnd)
{
	...
	/* Create a menu item for "Japanese Novel" */
	mi.fMask = MIIM_TYPE | MIIM_ID;
	mi.wID = ID_NEW_PROJECT_JAPANESE_NVL;
	mi.dwTypeData = bEnglish ? L"Japanese Novel" : L"日本語ノベル形式";
	InsertMenuItem(hMenuProject, nOrder++, TRUE, &mi);

	/* Create a menu item for "Japanese Vertical Novel" */
	mi.fMask = MIIM_TYPE | MIIM_ID;
	mi.wID = ID_NEW_PROJECT_JAPANESE_NVL_V;
	mi.dwTypeData = bEnglish ? L"Japanese Vertical Novel" : L"日本語縦書きノベル形式";
	InsertMenuItem(hMenuProject, nOrder++, TRUE, &mi);
	...
	/* Create a menu item for "Export a macOS project" */
	mi.wID = ID_EXPORT_MAC;
	mi.dwTypeData = bEnglish ?
		L"Export a macOS project" :
		L"macOSプロジェクトをエクスポートする";
	InsertMenuItem(hMenuExport, nOrder++, TRUE, &mi);
	EnableMenuItem(hMenu, ID_EXPORT_MAC, MF_GRAYED);

#if 0
	/* Create a menu item for "Export a Linux game" */
	mi.wID = ID_EXPORT_LINUX;
	mi.dwTypeData = bEnglish ?
		L"Export a Linux game" :
		L"Linuxゲームをエクスポートする";
	InsertMenuItem(hMenuExport, nOrder++, TRUE, &mi);
	EnableMenuItem(hMenu, ID_EXPORT_LINUX, MF_GRAYED);
#endif

	/* Create a menu item for "Export iOS project" */
	mi.wID = ID_EXPORT_IOS;
	mi.dwTypeData = bEnglish ?
		L"Export an iOS project" :
...
static VOID StartGame(void)
{
	...
	do {
		...
		EnableMenuItem(hMenu, ID_NEW_PROJECT_JAPANESE_NVL, MF_GRAYED);
		EnableMenuItem(hMenu, ID_NEW_PROJECT_JAPANESE_NVL_V, MF_GRAYED);
		EnableMenuItem(hMenu, ID_NEW_PROJECT_ENGLISH_ADV, MF_GRAYED);
		...
		EnableMenuItem(hMenu, ID_EXPORT_LINUX, MF_ENABLED);
...
static void OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case ID_NEW_PROJECT_JAPANESE_ADV:
		...
		break;
	case ID_NEW_PROJECT_JAPANESE_NVL:
		OnNewProject(L"games\\jp-nvl\\*");
		break;
	case ID_NEW_PROJECT_JAPANESE_NVL_V:
		OnNewProject(L"games\\jp-vert\\*");
		break;
...
const wchar_t *SelectFile(const char *pszDir)
{
	...
	assert(wcscmp(wszProjectDir, L"") != 0);
	...
	if (wcswcs(wszPath, wszBase) != wszPath)
	{
		MessageBox(hWndMain, bEnglish ?
				   ...
				   TITLE,
				   MB_ICONEXCLAMATION);
		log_error(conv_utf16_to_utf8(wszPath));
		log_error(conv_utf16_to_utf8(wszBase));
		log_error(conv_utf16_to_utf8(wszBase));
	}
...
static VOID OnExportWin(void)
{
	...
	{
		log_info(bEnglish ?
				 "Failed to export the package.pack." :
	...
	{
		log_info(bEnglish ?
				 "Failed to copy an exe file." :
				 "実行ファイルのコピーに失敗しました。");
		return;
	}

	/* movをコピーする */
	CopyMovFiles(L".\\mov\\*.wmv", L".\\windows-export\\mov");

	/* パッケージを移動する */
	if (!MovePackageFile(L".\\data01.arc", L".\\windows-export\\data01.arc"))
	{
		log_info(bEnglish ?
				 "Failed to move the data01.arc file." :
...
static VOID OnExportMac(void)
{
	...
	{
		log_info(bEnglish ?
				 "Failed to export the data01.arc file." :
				 "data01.arcのエクスポートに失敗しました。");
		return;
	}
	...
	{
		/* アプリをコピーする */
		if (!CopyLibraryFiles(L"tools\\engine.dmg", L".\\macos-export\\engine.dmg"))
		{
			log_info(bEnglish ?
					 "Failed to copy source files for macOS." :
					 "アプリのコピーに失敗しました。"
		...
		{
			log_info(bEnglish ?
					 "Failed to movet the data01.arc file." :
					 "data01.arcの移動に失敗しました。");
			return;
		}
...
/* Linux向けにエクスポートのメニューが押下されたときの処理を行う */
static VOID OnExportLinux(void)
{
	if (MessageBox(hWndMain, bEnglish ?
				   L"The process may take some time. Do you wish to proceed?\n" :
	...
	{
		log_info(bEnglish ?
				 "Failed to export the package.pack file." :
				 "data01.arcのエクスポートに失敗しました。");
		return;
	}

	/* フォルダを再作成する */
	RecreateDirectory(L".\\linux-export");

	/* ファイルをコピーする */
	if (!CopyLibraryFiles(L"tools\\onengine-linux", L".\\linux-export\\onengine"))
	{
		log_info(bEnglish ?
	...
	/* movをコピーする */
	CopyMovFiles(L".\\mov\\*.wmv", L".\\linux-export\\mov");

	/* パッケージを移動する */
	if (!MovePackageFile(L".\\data01.arc", L".\\linux-export\\data01.arc"))
	{
		log_info(bEnglish ?
				 "Failed to move the data01.arc file." :
				 "data01.arcの移動に失敗しました。");
	...
	/* Explorerを開く */
	ShellExecuteW(NULL, L"explore", L".\\linux-export", NULL, NULL, SW_SHOW);
}
...
static VOID OnExportWeb(void)
{
	...
	/* movをコピーする */
	CopyMovFiles(L".\\mov\\*.mp4", L".\\web-export\\mov");
...
static VOID OnExportAndroid(void)
{
				   ...
				   MB_ICONINFORMATION | MB_YESNO) == IDYES)
	{
		...
		MessageBox(hWndMain, bEnglish ?
				   L"If building fails, please move the game folder under C:\\." :
	...
	MessageBox(hWndMain, bEnglish ?
			   L"Will open the exported source code folder.\n"
			   L"Please build with Android Studio." :
			   L"エクスポートしたソースコードフォルダを開きます。\n"
...
static VOID OnExportIOS(void)
{
	...
	{
		log_info(bEnglish ?
				 "Failed to copy the source files for Android." :
				 "ソースコードのコピーに失敗しました。"
...
{
	if (MessageBox(hWndMain, bEnglish ?
				   L"Are you sure you want to export a package file?\n"
	...
	if (create_package("")) {
		log_info(bEnglish ?
				 "Successfully exported the data01.arc file." :
