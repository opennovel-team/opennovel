...
public class OpenNovelScript : MonoBehaviour
{
    ...
    private GameObject _audioBGM;
    private GameObject _audioSE;
    private GameObject _audioCV;
    private GameObject _audioSysSE;
    ...
    void Start()
    {
		...
		// Make sound streams.
		_audioBGM = new GameObject("BGM", typeof(OpenNovelAudio));
		_audioSE = new GameObject("SE", typeof(OpenNovelAudio));
		_audioCV = new GameObject("CV", typeof(OpenNovelAudio));
		_audioSysSE = new GameObject("SYSSE", typeof(OpenNovelAudio));
    ...
    {
        if (stream == 0)
            GameObject.Find("BGM").GetComponent<AudioSource>().volume = vol;
        else if (stream == 1)
            GameObject.Find("SE").GetComponent<AudioSource>().volume = vol;
        else if (stream == 2)
            GameObject.Find("CV").GetComponent<AudioSource>().volume = vol;
        else
            GameObject.Find("SYSSE").GetComponent<AudioSource>().volume = vol;
    }
    ...
    {
        if (stream == 0)
  		   return GameObject.Find("BGM").GetComponent<AudioSource>().isPlaying ? 1 : 0;
        else if (stream == 1)
            return GameObject.Find("SE").GetComponent<AudioSource>().isPlaying ? 1 : 0;
        else if (stream == 2)
            return GameObject.Find("CV").GetComponent<AudioSource>().isPlaying ? 1 : 0;
        else
            return GameObject.Find("SYSSE").GetComponent<AudioSource>().isPlaying ? 1 : 0;
    }
